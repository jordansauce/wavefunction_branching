...
# from instantaneous_benchmarks.generate_test_inputs import block_diagonal_matrices
...
# METHODS = {
#     "bell_discard_classical": (None,) + tuple(x for x in GRADDESC_METHODS if "discard_classical" in x),
#     "bell_keep_classical": (None,) + tuple(x for x in GRADDESC_METHODS if "discard_classical" not in x),
#     "vertical_svd_micro_bsvd": (None,) + tuple(x for x in GRADDESC_METHODS if "identical_blocks" not in x),
...
#     theta_orig = block_diagonal_matrices(block_sizes=[branch_chi, branch_chi], N=N, noise_introduced=noise_introduced) # shape: N, branch_chi, branch_chi
#     if identical_blocks:
...
#             print("\n\n" + methods_str)
#             theta_orig, theta_orig_purified, theta_scrambled, theta_scrambled_purified = generate_test_tensor(
#                 N = N,
...
#                 assert results["global_reconstruction_error_trace_distance"] < tolerance, methods_str
